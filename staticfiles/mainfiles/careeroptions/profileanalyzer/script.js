...
function renderPaginatedJobsAndControls(jobs, currentPage) {
    ...
    if (paginatedJobs?.length > 0) {
        ...
        paginatedJobs.forEach((profileJobs) => {
            ...
            <p>
                        <strong>Minimum Skills Required: </strong>${profileJobs[0]?.posts_data?.profile_masterdata
                    ?.minimum_skills_required
                    ? profileJobs[0]?.posts_data?.profile_masterdata
                        ?.minimum_skills_required
                    : ""
                } | 
                        <strong>Minimum Qualifications: </strong>${profileJobs[0]?.posts_data?.profile_masterdata
                    ...
                    : ""
                        ...
                        <strong>Preferred Streams: </strong>${profileJobs[0]?.posts_data?.profile_masterdata
                    ?.preferred_streams
                    ? profileJobs[0]?.posts_data?.profile_masterdata
                        ?.preferred_streams ||
                    ...
                    : ""
                        ...
                        <strong>Entrance Exam: </strong>${profileJobs[0]?.posts_data?.profile_masterdata
                    ?.entrance_exam
    ...
    for (const job of jobs_data) {
        if (
            ...
            (selectedIndustry === "all" ||
                job.posts_data?.industry_masterdata?.code === selectedIndustry) &&
            (selectedProfile === "all" ||
