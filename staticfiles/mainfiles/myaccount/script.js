...
async function fetchAndUseNames() {
    ...
    const twelfthDetails = `${twelfthSubjectName || ""} - ${userData.twelfthPercentage || ""
        }`;
    const diplomaDetails = `${diplomaStreamName || ""} - ${diplomaName || ""}`;
    const graduationDetails = `${graduationStreamName || ""} - ${graduationDegreeName || ""
        } - ${userData.graduationPercentage || ""}`;
    const pgDetails = `${pgStreamName || ""} - ${pgDegreeName || ""} - ${userData.pgPercentage || ""
        }`;
